...
function hive_transform_reclist2redisformat(){
    ...
    (
      ...
      from
      (
          select concat('arcf_001_iting_', user_id) as user_id, item_id, row_number() over (partition by user_id order by rank asc) as rank2
          from
          (
...
#    curl  "http://bdsp-manager.api.prd.bj01.vivo.lan:8080/excution/taskCode/VDLQCZGC/add.json?jdbc.reader.sqlArgs=$DATE,arcf_001&redis.writer.keyTimeout=1000000000"
    curl  "http://bdsp-facade.api.prd.bj01.vivo.lan:8080/bdsp/api.json?taskCode=WHUNHFTD&methodType=addExecution&ds=$DATE"
